...
//                    $link_page .= '<span class="current">' . $page . '</span>';
        ...
        $listRows_str = '<li class="paginate_button previous "><select id="listRows" style="height: 30px;line-height:  30px;display: '.$display.'">';
        $page = I('get.p');
        foreach ($listRows_arr as $k=>$v){
            $selected = $listRows == $v?'selected':'';
//              $selected = $this->listRows == $v?'selected':'';
        ...
        $listRows_str .= " <script>$('#listRows').change(function () {var rows = $(this).val();ajax_get_table('search-form2',".$page.",rows); })</script>";
        /**
         * 自定义每页显示几条
         */
