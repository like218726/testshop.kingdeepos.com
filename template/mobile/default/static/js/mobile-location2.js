...
$('body').on('click', '.area-list p', function () {
  ...
  var id = '#'+$('#addr-container').attr('data-id');
  $(id).text(province_name+city_name+district_name);
  id = '#'+$('#addr-container').attr('data-iid');
  $(id).val(province_name+city_name+district_name);
});
//标记字体颜色
...
// 设置省市区的id
function set_area_id(addComp){
  var province_id = get_province_id(addComp.province);

  if (province_id == 1){
    if(addComp.district.indexOf('区')>0){
      addComp.city = '市辖区';
    }else{
      addComp.city = '县';
    }
  }
  var city_id = get_city_id(addComp.city,province_id,addComp.district);

  setCookies('province_name',addComp.province,30*24*60*60*1000);
  setCookies('city_name',addComp.city,30*24*60*60*1000);
  setCookies('district_name',addComp.district,30*24*60*60*1000);

  // 省市能直接获取，区要请求接口，有时间差
  setCookies('province_id',province_id,30*24*60*60*1000);
  setCookies('city_id',city_id,30*24*60*60*1000);
  get_district_id(addComp.district,city_id);
}
// 获取省id
function get_province_id(name){
  for(var o in iplocation){
    if(o == name)
      return iplocation[o].id
  }
  return '';
}
//获取市id 如果没获取到，就默认第1个
function get_city_id(name,province_id,district){
  if(province_id=='') return '';
  var list = provinceCityJson[province_id];
  for(var o in list){
    if(list[o].name == name)
      return list[o].id
  }
...
//获取区id
function get_district_id(name,city_id){
  if(city_id=='') return '';
  $.ajax({
    type: "POST",
    url: "/index.php/Home/Api/getRegionIdByName",
    data: {"name":name,"city_id":city_id},
    dataType: "json",
    success: function (res) {
      console.log(res)
      if(res.id != 0){
        setCookies('district_id',res.id,30*24*60*60*1000);
      }
      locationInitialize();
    },error:function(){
      locationInitialize();
