...
function ajaxDispatching() {
  ...
  if(typeof (auto_bd) != 'undefined'  && auto_bd==2 ){
    // 定位结束
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == 1) {
          ...
          joinCart(false)
          if(data.is_not==1){
              $('#join_cart.dis_btn').removeClass('dis')
          }
          $('#shipping_freight').text(data.msg);
        ...
        if(typeof (auto_bd) != 'undefined'  && auto_bd==3 ){
...
function getProvince(){
    ...
    $.ajax({
        ...
        dataType:"json",
        success: function(data){
            var province_html = province_html2 ='';
            if(data.status == 1){
                var province_count = data.result.length;
                for(var i = 0;i < province_count;i++){
                    province_html += '<p data-id="'+data.result[i].id+'">'+data.result[i].name+'</p>';
                    province_html2 += '<div class="sel_info" data-id="'+data.result[i].id+'" ><div class="sel_pri_name"> '+data.result[i].name+'</div><div ><i class="Mright Mrightt" style="float:right" ></i></div></div>';
                }
            }

            $(".province-list").empty().append(province_html).show();
            $('.province-list').siblings('div').hide();
            $(".province-list2").empty().append(province_html2).show();
