...
/***********************************************************************************************************/
    ...
    function img_call_back(fileurl_tmp){
        $('#tpd-mobile-views').children().each(function(){
            if($(this).hasClass('tpd-editing')){
                $(this).css('height','auto')
                $(this).children().find('#h-bgimg').css('width','auto')
                var that = $(this);
                setTimeout(function () {
                    var ptcw = (that.children().find('#h-bgimg')[0].naturalWidth/23.4375/2).toFixed(2)+'rem';
                    var ptch = (that.children().find('#h-bgimg')[0].naturalHeight/23.4375/2).toFixed(2)+'rem';
                    that.children().find('#h-bgimg').css('width',ptcw)
                    that.children().find('#h-bgimg').css('height',ptch)
                })
            }
        })
        ...
        //底部
        if(choice_type == '11'){
            footer_item=parseInt($(gol).parents('li').index())-1;
...
function is_aready(block_type){
    for (var k in block) {
        if(typeof (block[k].block_type) && block[k].block_type == block_type){
            $('#guide-box').remove();
            return true;
        }
    }
...
function createModule(eidtid){
    if(eidtid==10 && is_aready('10')){
        layer.msg('只能添加一个滚动公告', {icon: 2, time: 1000});
        return false;
    }
    if(eidtid==18 && is_aready('18')){
        layer.msg('只能添加一个多门店店铺', {icon: 2, time: 1000});
        return false;
    }
    if(eidtid==11 && is_aready('11')){
        layer.msg('只能添加一个底部菜单', {icon: 2, time: 1000});
        return false;
    }
    if(eidtid==3 && is_aready('3')){
        layer.msg('只能添加一个商品列表', {icon: 2, time: 1000});
        return false;
    }
    ...
    * 10公告    11底部菜单   12新闻  13轮播多图  14视频  15图文列表  16富文本  17单文本  18多门店店铺 19智能表单
    */
    ...
    if(eidtid=='11'){   //生成底部板块时默认底部风格为0
        ...
        block[divid]['nav']=[
            {
                "pic1":"/template/mobile/default/static/images/home1.png",
                "pic2":"/template/mobile/default/static/images/home2.png",
                "title_name":"首页",
                "url":"/index.php/Mobile/Index/index"
            },
            {
                "pic1":"/template/mobile/default/static/images/category1.png",
                "pic2":"/template/mobile/default/static/images/category2.png",
                "title_name":"分类",
                "url":"/index.php/Mobile/Goods/categoryList"
            },
            {
                "pic1":"/template/mobile/default/static/images/find1.png",
                "pic2":"/template/mobile/default/static/images/find2.png",
                "title_name":"发现",
                "url":"/index.php/Mobile/Goods/ajaxSearch"
            },
            {
                "pic1":"/template/mobile/default/static/images/cart1.png",
                "pic2":"/template/mobile/default/static/images/cart2.png",
                "title_name":"购物车",
                "url":"/index.php/Mobile/Cart/index"
            },
            {
                "pic1":"/template/mobile/default/static/images/user1.png",
                "pic2":"/template/mobile/default/static/images/user2.png",
                "title_name":"我的",
    ...
    if(eidtid=='13'){
        ...
        block[divid].name_show=1;   //列表名称显示  0不显示  1显示
        block[divid].order=0;       //商品排序
        block[divid].tab_title="最新上线";
        show_edit13(block[divid]);  // 改为依数据显示页面
    }
    ...
    if(eidtid=='19'){
        block[divid].bg_color = '#ffffff'; // 背景颜色 白
        block[divid].form_name = '智能表单'; // 表单名称
        block[divid].submit_name = '提交'; // 提交按钮文字
        block[divid].submit_bg_color = '#ff0000'; // 按钮背景颜色 红
        block[divid].submit_color = '#ffffff'; // 按钮文字颜色 白
        block[divid].title_color = '#000000'; // 标题颜色 黑
        block[divid].text_color = '#cccccc'; // 输入的文字颜色 灰
        block[divid].result = '提交成功'; // 提交反馈

        block[divid].nav = [];
        block[divid].nav.push(init_a_form_option(0));
...
var dragM='<div style="float: left" class="tpd-guide-box" id="guide-box">模块将移动到此处</div>';
/*碰撞检测*/
...
function dragModlue(){
    ...
    //底部专用点击事件
     $(document).on('mousedown','#tpd-mobile-views .tpdm-footer-false',function (e){
         var clr = $(this).find('.tpd-footer').children('a').eq(0).children('span').css('color');
         function ct(color) {
             var rgb = color.split(',');
...
function li_split(){
    ...
    for(var i=0;i<block[divid].nav.length;i++){
        if(block[divid].nav[i].url_title){
            var url_title = block[divid].nav[i].url_title;
        }else{
            var url_title = '请选择';
        }
        var addlink = url_title=='创建链接'?'tpd-nav-addlink':'';
        var addlink_name = block[divid].nav[i].show_goods_name?block[divid].nav[i].show_goods_name:'添加链接目标';
        ...
        li_str+='<div class="tpd-select-wrap"><span>'+url_title+'</span>';
        ...
        li_str+='</div><div class="tpd-addlink-target '+addlink+'"><i></i>'+addlink_name+'</div></div></div> <div class="c-suggest"><p>建议尺寸宽度80px，高度80px</p></div>';
        li_str+='<div class="tpd-cdiy-identity"><img src="/public/static/images/ico-diy-drag.png" alt="" /></div>'
...
<li>
    <div class="tpd-foot_tbxz">
        <div class="tpd-nav-ico tpd-foots-ico"><img src="/template/mobile/default/static/images/home1.png" alt="" /></div>
        <div class="tpd-nav-ico tpd-foots-ico"><img src="/template/mobile/default/static/images/home2.png" alt="" /></div>
    </div>
...
function footer_split(){
    var li_str='<div class="tpd-cstyle-select tpd-cdiy-font28 h-custom-color">字体颜色：<input id="h-custom-color"  type="color" name="color" class="h-color" value="#ff354d"></div>';
    for(var i=0;i<block[divid].nav.length;i++){
        ...
        if(block[divid].nav[i].url){
            if(block[divid].nav[i].show_goods_name){
                var show_goods_name = block[divid].nav[i].show_goods_name;
            }else{
                var show_goods_name =  block[divid].nav[i].url;
            }
            // li_str+='<div class="tpd-addlink-target" style="display:block;"><i></i>'+block[divid].nav[i].url+'</div>';
            li_str+='<div class="tpd-addlink-target" style="display:block;"><i></i>'+show_goods_name+'</div>';
        }else{
    ...
    $('#h-custom-color').change(function () {
        $('.tpd-footer .footmenu-ac>span').css('color',$(this).val())
        block[divid].navFontColor = $(this).val()
    })
}
...
function roll_split(){
    ...
    for(var i=0;i<block[divid].nav.length;i++){
        ...
        if(block[divid].nav[i].pic){
            ...
            str+='<span></span><label>建议宽度750px，高度340px<br />如尺寸不达标，图片将自适应显示</label>';
        }
        str+='</div>';
        ...
        str+='<a class="tpd-del-nav1" href="javascript:;"></a><a class="tpd-del-nav" href="javascript:;"></a><a class="tpd-del-nav2" href="javascript:;"></a></li>';
    }
    $('#roll').html(str);
}
...
function goods_list_edit(){
    ...
    for(var i=0;i<block[divid].nav.length;i++){
        ...
        str+='</dl></div>';
        str+='<div class="tpd-cdiy-identity"><img src="/public/static/images/ico-diy-drag.png" alt="" /></div>';
...
/*************************************************弹窗部分点击底部链接目标显示已有的数据***********************************************************/
function show_popLink(obj,index){
    var i=$(link).parents('li').index();
    var pop = 0;
    var url_item = 0;
    switch (Number(choice_type)){
        case 2:
             pop = $('.tpd-edits-hidden.tpd-editing').find('.tpdm-navstyle a').eq(i);
             url_item = pop.attr('url_item');
            break;
        case 11:
             pop = $('.tpdm-footer-false.tpdm-footf-edit').find('a').eq(i);
             url_item = pop.attr('url_item');
            break;
    }

    if(!pop){ return;}
    if(index != url_item){ return;}
    switch (index)
    {
        case 0:
            $("#url1").val(pop.attr('url_id'));
            break;
        case 1:
            $("#shop_link").find("input").each(function () {
                if($(this).val() == pop.attr('url_id')){
                    $(this).attr('checked',true)
                }
            });
            break;
        case 2:
            get_category(pop.attr('cat1'),'cat_id_2',pop.attr('cat2'));
            get_category(pop.attr('cat2'),'cat_id_3',pop.attr('cat3'));
            goods_class();
            $("#cat_id").val(pop.attr('cat1'));
            $("#url_show").text(pop.attr('url_id'));
            break;
        case 3:
            $(obj).find('.p-goods-list .p-goods-scroll').find('ul').each(function () {
                if($(this).children().eq(0).find('input').val() == pop.attr('url_id')){
        ...
        case 4:
            $(obj).find("input").each(function () {
                if($(this).attr('template-id') == pop.attr('url_id')){
                    $(this).attr('checked',true)
                }
            });
            break;
        case 5:
            $(obj).find(".p-goods-item").each(function () {
                if($(this).children().eq(0).find('input').val() == pop.attr('url_id')){
                    $(this).children().eq(0).find('input').attr('checked',true)
                }
            });
...
// 第一次弹出时，要全没有选，如果是加载更多，则不要全没选,所以
function show_goods_hand1(){
    $(".icon-select.j-select").removeClass('cur')
    var goods = $("#pop-link2").attr('goods')
    if(goods.indexOf(',')>-1){
        var arr = goods.split(',')
        for (var i = arr.length - 1; i >= 0; i--) {
            $(".icon-select.j-select.goods"+arr[i]).addClass('cur')
        }
    }else if(goods){
        $(".icon-select.j-select.goods"+goods).addClass('cur')
    }
}
function set_goods_1(v){
    $("#pop-link2").find('input').each(function(){
        if($(this).val() == v){
...
$(function(){
    $("body").on('click','.tpd-addlink-target',function(){
        ...
        switch (Number(choice_type)){
            case 2:
                var url_item = $('.tpd-edits-hidden.tpd-editing').find('.tpdm-navstyle a').eq(i).attr('url_item');
                break;
            case 11:
                var url_item = $('.tpdm-footer-false.tpdm-footf-edit').find('a').eq(i).attr('url_item');
                break;
        }
        if(0 == url_item){
            //选中了哪些链接目标，渲染出来、
    ...
    $("body").on('click','.hand1',function(){
        ...
        $(".lists-pop-bg").show();
        // goods   "40,41,"
        $('#pop-link2').attr('goods','')
        if(block[divid].block_type == '13'){
            //轮播
            if(typeof block[divid].goods != 'undefined'){
                $('#pop-link2').attr('goods',block[divid].goods)
            }
        }else{
            // =3
            var i = $(this).attr('data-i');
            if(typeof block[divid].nav[i].sql_where.goods != 'undefined'){
                $('#pop-link2').attr('goods',block[divid].nav[i].sql_where.goods)
            }
        }
        //要么请求数据，要么显示选中
         is_page=1
