...
/***********************************************************************************************************/
    ...
    var is_son=0;       //记录店铺底部打开的弹窗是否属于子分类的   1表示打开的子分类
    var footer_tab=0;   //底部模式下操作的导航序号
    //var add_item='';    //这特么是个坑 底部二级分类克隆的坑
    ...
    //店铺页面底部菜单导航名称修改
    function footer_tit_name(b){
        var i=$(b).parents('li').index();
        //alert(i);return false;
        var viewUl=$('.tpdm-footf-js').children('div');
        viewUl.children('div').eq(i).find('span').find('a').html($(b).val());
        block.footer['nav'][i].title_name=$(b).val();
    }

    
    $(".tpd-add-tab").on('blur','.tpd-add-tabs input',function  () {
        var i= $(this).parents(".tpd-add-tabs").index();
        block.footer['nav'][footer_tab]['son'][i].title_name=$(this).val();
    })
...
if(info){
    ...
    if($('#is_index').val()){
        block.is_header=1;  //是否已存在店铺头部 0不存在  1存在
        block.is_footer=1;  //是否已存在底部板块 0不存在  1存在
    }else{
    ...
    block.header_type=0;//默认店铺头部样式为0
    block.footer_type=0;//默认店铺底部样式为0
    block['footer']={};
    block.footer['nav']=[
        {
            "title_name":"宝贝分类",
...
function createModule(eidtid){
    ...
    if(eidtid==8){ //搜索栏
        if(block.is_search==1){
            $('#guide-box').remove();
            layer.msg('只能设置一个搜索栏', {icon: 2, time: 1000});
            return false;
        }
        $('.tpdm-head-wrap>.tpdm-head-js').addClass('tpdm-head-scale').html(html);
        $('.tpd-height').toggleClass('tpdm-head-edit').show();
        ...
        if(block.is_header==1){
            $('#guide-box').remove();
            layer.msg('只能设置一个店铺头部', {icon: 2, time: 1000});
            return false;
        }
        //$('.tpd-null >.tpdm-head-jsa').addClass('tpdm-head-jsa').html(html);
    }else{
...
$(function(){
    $("body").on('click','.tpd-addlink-target',function(){
        ...
        if(choice_type==11){    //判断是否是通过二级分类打开的弹窗
            var parent_tmp=$(this).parent('div').parent('div');
            if((parent_tmp).hasClass('tpd-add-tabs')){
                is_son=1;   //若是二级分类打开的弹窗此处标记is_son为1
            }
        }
