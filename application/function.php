...
function get_secondary_prefix(){
    ...
    $site_domain = tpCache('subdomain.site_domain');
...
/**
 * 默认空图片
 * @param int $app | 如果要域名http,传1，默认无域名
 * @return string
 */
function thumb_empty($app=0){
    $url = '/public/upload/img/thumb_empty.png';
    // 从旧的升级新的使用
    if(!file_exists('.'.$url)){
        if(!file_exists('./public/upload/img')){
            mkdir('./public/upload/img', 0777, true);
        }
        $png = file_get_contents('./public/images/icon_goods_thumb_empty_300.png');
        file_put_contents('.'.$url,$png);
    }
    if(!empty($app)) $url = SITE_URL . $url;
    return $url;
}
