...
//$page = $pager->show();//分页显示输出
        ...
        foreach ($list as $k => $v) {
            ...
            if ($v['source'] == 0) {
                $list[$k]['publisher'] = "总平台";
            } else if ($v['source'] == 1) {
                $publisher = M('store')->where('user_id', $v['user_id'])->value('store_name');
                $list[$k]['publisher'] = $publisher;
            } else if ($v['source'] == 2) {
                $publisher = db('users')->where('user_id', $v['user_id'])->field('realname, mobile')->find();
                $list[$k]['publisher'] = $publisher['realname'] ? $publisher['realname'] : $publisher['mobile'];
            }
        }
    ...
    public function aticleHandle()
    {
        ...
        if ($data['act'] == 'add') {
            ...
            $data['add_time'] = time();
            $r = M('news')->add($data);
        } elseif ($data['act'] == 'edit' || $data['act'] == 'audit') {
    ...
    public function delNewsList()
    {
        ...
        $this->ajaxReturn(['status' => 1, 'msg' => '操作成功', 'url' => U("Admin/News/newsList")]);
    }

    public function news_comment()
    {
        $article_id = input('article_id');
        $Comment = db('news_comment');
        $p = empty($_REQUEST['p']) ? 1 : $_REQUEST['p'];
        $size = empty($_REQUEST['size']) ? 20 : $_REQUEST['size'];
        ...
        $article_id && $where .= " and article_id = $article_id ";

        $list = $Comment->where($where)->order('comment_id desc')->page("$p,$size")->select();
        $count = $Comment->where($where)->count();// 查询满足要求的总记录数
        $pager = new Page($count, $size);// 实例化分页类 传入总记录数和每页显示的记录数

        foreach ($list as $k => $v) {
            $commenter = db('users')->where('user_id', $v['user_id'])->field('realname, mobile')->find();
            $list[$k]['commenter'] = $commenter['realname'] ? $commenter['realname'] : $commenter['mobile'];
    ...
    public function commentHandle()
    {
        ...
        $result = db('news_comment')->where('comment_id', $comment_id)->update(['check_type' => $check_type]);
