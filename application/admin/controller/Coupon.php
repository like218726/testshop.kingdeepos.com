...
use think\Page;
use think\Db;
use app\admin\validate\Coupon as ValiCoupon;
use think\Loader;
...
class Coupon extends Base {
    ...
    public function index(){
        //获取优惠券列表
        $map = array('store_id'=>array('<>',0),'type'=>array('<>',4));
        $name = I('name');
    ...
    public function addEditCoupon()
    {
        ...
        if (empty($data['id'])) {
            ...
            if ($data['use_type'] == 2) {
                ...
                Db::name('goods_coupon')->add(['coupon_id' => $row, 'goods_category_id' => $cat_id]);
            }
        } else {
            ...
            if ($data['use_type'] == 2) {
                ...
                Db::name('goods_coupon')->add(['coupon_id' => $data['id'], 'goods_category_id' => $cat_id]);
            }
        }
        if ($row !== false) {
