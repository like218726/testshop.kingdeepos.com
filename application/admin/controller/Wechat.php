...
//        );
            ...
            if($c_menus){
                foreach($c_menus as $kk=>$vv){
                    ...
                    if($add['type'] == 'click'){
                        ...
                        $wxPay = db('plugin')->where(array('type'=>'payment','code'=>'miniAppPay'))->find();
                        $wxPayVal = unserialize($wxPay['config_value']);
                        $add['appid'] = $wxPayVal['appid'];
                    }else{
                        $add['key'] = $vv['value'];
                    }
                ...
                if($new_arr[$count]['type'] == 'click'){
                    ...
                    $wxPay = db('plugin')->where(array('type'=>'payment','code'=>'miniAppPay'))->find();
                    $wxPayVal = unserialize($wxPay['config_value']);
                    $new_arr[$count]['appid'] = $wxPayVal['appid'];
                }else{
                    //其他事件类型
                    $new_arr[$count]['key'] = $v['value'];
                }
    ...
    public function fans_list()
    {
        ...
        if($keyword){
            ...
            $arr = $wechatObj->getFanInfo($keyword);
            $page  = new Page(1, 10);
            if($arr){
                $list[] = $arr;
            }else{
