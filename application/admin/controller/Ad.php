...
class Ad extends Base{
    public function ad(){       
        ...
        if($ad_id){
            ...
            if($ad_id['end_time']<time()){
                unset($ad_info);
                $ad_info['ad_id']=$ad_id;
                $ad_info['start_time']=time();
                $ad_info['end_time']=time()+2592000;
            }
    ...
    public function adHandle(){
    	...
    	if($r){
            ...
            if($redirect_url){
                session("ad_request_url",null);
                $redirect_url = urldecode($redirect_url);
                $redirect_url = urldecode($redirect_url);
                $redirect_url = urldecode($redirect_url);
                $this->success("操作成功",U('Admin/Ad/editAd' , array('request_url'=>urlencode($redirect_url) )));
            }
