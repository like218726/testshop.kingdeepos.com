...
class WechatLogic
{
    ...
    private function handleSubscribeEvent($msg)
    {
        ...
        if (!Db::name('oauth_users')->where('openid', $openid)->find()) {
            ...
            $userData['first_leader'] = substr($msg['EventKey'], strlen('qrscene_'));
            $userData['first_leader'] = $userData['first_leader']?$userData['first_leader']:0;
            //缓存关注时候传过来的上级id，User/bind_guide用到
            Cache::set($openid,$userData['first_leader'],86400);
        }
    ...
    public function getDefaultTemplateMsg($template_sn = null)
    {
        ...
        $valid_sns = ['OPENTM204987032', 'OPENTM202243318','OPENTM407456411']; //目前支持的模板
        $valid_templates = [];
     ...
     * @return array
     */
    public function sendTemplateMsgOnTeam($found_id){
        if ( ! $found_id) {
        ...
        //拼团成功的模板消息通知只发送给已支付的参团人和开团人
        $follow_user_ids = Db::name('team_follow')->where(['found_id'=>$found_id,'status'=>1])->select();
        ...
        $follow_user_ids_data = $follow_user_ids[0];
        $follow_user_ids_data['follow_user_id'] = $follow_user_ids_data['found_user_id'];
        $follow_user_ids_data['order_id'] = Db::name('team_found')->where('found_id',$found_id)->value('order_id');
        $follow_user_ids[] = $follow_user_ids_data;
        foreach ($follow_user_ids as $user){
            $openid = Db::name('oauth_users')->where(['user_id' => $user['follow_user_id'], 'oauth' => 'weixin', 'oauth_child' => 'mp'])->value('openid');
            if($openid){
                $data['keyword1']['value'] = Db::name('order')->where('order_id', $user['order_id'])->value('order_sn');
                $goods = Db::name('order_goods')->where('order_id', $user['order_id'])->value('goods_name');
                $data['keyword2']['value'] = $goods;
                $url = SITE_URL.url('/mobile/order/team_detail?order_id='.$user['order_id']);
                $return = self::$wechat_obj->sendTemplateMsg($openid, $tpl_msg->template_id, $url, $data);
            }
        }
        if ($return === false) {
            return ['status' => -1, 'msg' => self::$wechat_obj->getError()];
    ...
    public function sendTemplateMsgOnPaySuccess($order)
    {
        ...
        if(empty($order['pay_time'])) $order['pay_time'] = time();
        if(empty($order['order_amount']) && !empty($order['user_money'])) $order['order_amount'] = $order['user_money'];
        $data = [
            'first' => ['value' => '订单支付成功！'],
            ...
            'keyword5' => ['value' => $order['total_amount']],
            'remark' => ['value' => $tpl_msg->remark ?: ''],
        ];
