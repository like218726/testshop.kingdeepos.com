...
class PreSellLogic extends Prom
{
    ...
    public function initProm()
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->preSell = PreSellModel::get($this->specGoodsPrice['prom_id'],'',false);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->preSell = PreSellModel::get($this->goods['prom_id'],'',false);
        }
        if ($this->preSell) {
    ...
    public function buyNow($goods_num)
    {
        ...
        //兼容其他地方传的值是整数而不是数组
        $goods_num = isset($goods_num['goods_num']) ?$goods_num['goods_num']:$goods_num;
