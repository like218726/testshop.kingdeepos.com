...
class Coupon extends Model
{
    ...
    public function getCouponImgAttr($value, $data)
    {
        $url = '';
        if($data['goods_id'] && $data['use_type'] == 1){
            //遍历图片
            $url = goods_thum_images($data['goods_id'],400,400);
            if (strpos($url, 'http') !== 0) {
                $url = SITE_URL . $url;
            }
        }
        if( $data['goods_category_id']  && $data['use_type'] == 2){
            //遍历分类
            $url = db('goods_category')->where(['id'=>$data['goods_category_id']])->cache(true,60)->value('image');
            if (strpos($url, 'http') !== 0) {
                $url = SITE_URL . $url;
            }
        }
