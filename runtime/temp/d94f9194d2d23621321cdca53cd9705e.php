<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:47:"./application/admin/view/system/edit_right.html";i:1587634374;s:79:"/home/wwwroot/testshop.kingdeepos.com/application/admin/view/public/layout.html";i:1587634374;}*/ ?>
<!doctype html>
<html>
<head>
					...
					<table>
						...
						<?php if(is_array($info[right]) || $info[right] instanceof \think\Collection || $info[right] instanceof \think\Paginator): if( count($info[right])==0 ) : echo "" ;else: foreach($info[right] as $key=>$vo): ?>
							<tr id="<?php echo str_replace('@','_',$vo); ?>">
								<td><input name="right[]" type="text" value="<?php echo $vo; ?>" class="form-control" style="width:300px;" readonly></td>
								<td style="text-align: center;"><a class="ncap-btn" href="javascript:;" onclick="$(this).parent().parent().remove();updateActCheck();">删除</a></td>
							</tr>
						<?php endforeach; endif; else: echo "" ;endif; ?>
						</tbody>
					</table>
...
<script type="text/javascript">
    function chkbox_bind(){
        $('input:checkbox').change(function () {
            ...
            if(is_check){
                ...
                if(ncode!= ''){
                    strtr += '<td><input type="text" name="right[]" value="'+ncode+'@'+ $(this).val()+'" class="form-control" style="width:300px;" readonly></td>';
                }else{
                    strtr += '<td><input type="text" name="right[]" value="" class="form-control" style="width:300px;" readonly></td>';
                }
                strtr += '<td style="text-align: center;"><a href="javascript:;" class="ncap-btn" onclick="$(this).parent().parent().remove();updateActCheck();">删除</a></td>';
    ...
    function adsubmit(){
        ...
        if($('input[name="right[]"').eq(0).val() == ''){
            layer.msg('权限码不能为空！', {icon: 2,time: 1000});
            return false;
        }
        $.ajax({
            type: "POST",
            url: "<?php echo U('System/edit_right'); ?>",
            ...
            success: function (res) {
                ...
                if (res.status == 1) {
                    layer.msg(res.msg,{icon: 1,time: 2000},function(){
                    	location.href = "<?php echo U('System/right_list'); ?>";
                    });
                } else {
