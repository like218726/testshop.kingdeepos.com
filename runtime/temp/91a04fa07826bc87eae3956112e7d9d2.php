...
<div class="shopingcar-index fr">
	...
	<div class="nav tpshop2_0_nav p">
		...
		<script>
			...
			<?php if(is_array($pre_sell_list) || $pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator): if( count($pre_sell_list)==0 ) : echo "" ;else: foreach($pre_sell_list as $k=>$v): if($v['sell_end_time'] < time()): ?>
					var text<?php echo $v[goods_id]; ?>_<?php echo $v[item_id]; ?> = "活动已结束";
			<?php elseif(($v['sell_end_time'] > time()) AND ($v['sell_start_time'] < time())): ?>
				var text<?php echo $v[goods_id]; ?>_<?php echo $v[item_id]; ?> = "预售中 剩余" + GetRTime('<?php echo date("Y/m/d H:i:s",$v['sell_end_time']); ?>');
			<?php else: ?>
				var text<?php echo $v[goods_id]; ?>_<?php echo $v[item_id]; ?> = "预热中 剩余" + GetRTime('<?php echo date("Y/m/d H:i:s",$v['sell_start_time']); ?>');
			<?php endif; ?>
				$("#surplus_text<?php echo $v[goods_id]; ?>_<?php echo $v[item_id]; ?>").text(text<?php echo $v[goods_id]; ?>_<?php echo $v[item_id]; ?>);
			<?php endforeach; endif; else: echo "" ;endif; ?>
			}
			...
			<div class="w1224">
				<div class="stsho pre-sts">
					<div class="shop-list-splb pre-set-suma p">
						<ul>
							<?php if(empty($pre_sell_list) || (($pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator ) && $pre_sell_list->isEmpty())): ?>
								<p class="ncyekjl" style="font-size: 16px;margin:100px auto;text-align: center;">--暂无预售商品--</p>
								<?php else: if(is_array($pre_sell_list) || $pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator): $i = 0; $__LIST__ = $pre_sell_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
										<li>
											<div class="s_xsall">
												<div class="xs_img">
													<a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$vo['goods_id'],'item_id'=>$vo['item_id'])); ?>">
														<img class="lazy-list" data-original="<?php echo goods_thum_images($vo['goods_id'],262,262,$vo['item_id']); ?>" />
													</a>
												</div>
												...
												<div class="price-tag">
													<span class="now"><em class="li_xfo">￥</em><em><?php echo $vo['ing_price']; ?></em></span>
													<span class="pre-per">已购<em><?php echo $vo['deposit_goods_num']; ?></em>件</span>
													<div class="sjs-olde p">
														<p class="djs-cctime" ><span id="surplus_text<?php echo $vo[goods_id]; ?>_<?php echo $vo[item_id]; ?>"></span></p>
													</div>
												</div>
											</div>
										</li>
