        ...
        <div class="main-content" id="mainContent">
            ...
            <table class="ncsc-default-table">
			    ...
			    <?php else: if(is_array($stock_list) || $stock_list instanceof \think\Collection || $stock_list instanceof \think\Paginator): $i = 0; $__LIST__ = $stock_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
                    <tr class="bd-line">
                        <td></td>
                        <td><?php echo $v['goods_id']; ?> <?php echo $v['item_id']; ?></td>
                        <td><?php echo getSubstr($v['goods_name'],0,30); ?></td>
                        <td><?php echo (isset($v['key_name']) && ($v['key_name'] !== '')?$v['key_name']:"无"); ?></td>
                        <td><?php echo $v['shop_price']; ?></td>
                        <td><?php echo (isset($goods[$v[goods_id]]['supplier_name']) && ($goods[$v[goods_id]]['supplier_name'] !== '')?$goods[$v[goods_id]]['supplier_name']:'-'); ?></td>
                        <td>
                        <input class="txt-cen" type="text" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" 
                        onchange="updateStock(this)" old-stock="<?php echo $v['store_count']; ?>" data-goodsid="<?php echo $v['goods_id']; ?>" data-id="<?php echo $v['item_id']; ?>" value="<?php echo $v['store_count']; ?>" <?php if($v['prom_type'] != 0 OR $v['root_goods_id'] != 0): ?>disabled<?php endif; ?> />
                        </td>
...
<script>
    ...
    function updateStock(obj){
        if($(obj).val() == $(obj).attr('old-stock')){
            return;
        }
        $.ajax({
            type:'GET',
            url:"<?php echo U('Goods/updateGoodsStock'); ?>",
            data:{item_id:$(obj).attr('data-id'),store_count:$(obj).val(),old_stock:$(obj).attr('old-stock'),goods_id:$(obj).attr('data-goodsid')},
            dataType:'json',
            success:function(data){
                if(data.status == 1){
                    $(obj).attr('old-stock',$(obj).val());
