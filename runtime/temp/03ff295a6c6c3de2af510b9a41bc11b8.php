<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:41:"./application/admin/view/system/cash.html";i:1587634374;s:79:"/home/wwwroot/testshop.kingdeepos.com/application/admin/view/public/layout.html";i:1587634374;}*/ ?>
<!doctype html>
<html>
<head>
        ...
        <div class="ncap-form-default">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    ...
                    <p class="notic">（注：单笔最高提现额,,为0时则不限）</p>
                </dd>
            </dl>
    ...
    function fromsubmit(){
        ...
        if (cash_open == 1) {
            if(isNaN(min_cash) || isNaN(max_cash) || isNaN(cash_times) || isNaN(count_cash) || isNaN(service_ratio) || isNaN(min_service_money) || isNaN(max_service_money)){
                layer.msg('选项不能为空且必须为数字!', {icon: 2,time: 2000});
                return false;
            }
            if (service_ratio < 0) {
                layer.msg('手续费比例必须大于等于0!', {icon: 2,time: 2000});
                return false;
            }
            if (service_ratio >= 100) {
                layer.msg('手续费比例必须小于100!', {icon: 2,time: 2000});
                return false;
            }
            if (min_service_money > 0 && min_service_money >= min_cash) {
                layer.msg('最低手续费必须小于最低提现额!', {icon: 2,time: 2000});
                return false;
            }
            if (max_service_money > 0 && min_service_money >= max_service_money && min_service_money > 0) {
                layer.msg('最低手续费不得大于最高手续费!', {icon: 2,time: 2000});
                return false;
            } else {
                if(min_service_money < 0 || max_service_money < 0){
                    layer.msg('最低或最高手续费不得小于0!', {icon: 2,time: 2000});
                    return false;
                }
            }

            if(max_cash > 0 && min_cash >= max_cash && min_cash > 0){
                layer.msg('最低提现额必须小于最高提现额!', {icon: 2,time: 2000});
                return false;
            } else {
                if(min_cash < 0 || max_cash < 0){
                    layer.msg('最低或最高提现额不得小于0!', {icon: 2,time: 2000});
                    return false;
                }
            ...
            if (max_cash < 0) {
                ...
                layer.msg('最高提现额必须大于等于0!', {icon: 2,time: 2000});
                return false;
            }

            if (count_cash > 0 && max_cash > 0 && cash_times > 0) {
                if((max_cash * cash_times) > count_cash){
                    layer.msg('提现次数及最高提现额运算结果不得大于每日累计提现额度!', {icon: 2,time: 2000});
                    return false;
                }
            }

            if(count_cash < 0){
                layer.msg('每日累计提现额不能小于0!', {icon: 2,time: 2000});
                return false;
            }   
        }
		
		//过滤数字前面的0
		$('#min_cash').val(min_cash);
        $('#max_cash').val(max_cash);
        $('#cash_times').val(cash_times);
        $('#count_cash').val(count_cash);
        $('#service_ratio').val(service_ratio);
        $('#min_service_money').val(min_service_money);
        $('#max_service_money').val(max_service_money);
