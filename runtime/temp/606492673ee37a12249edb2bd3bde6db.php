   ...
   <div id="sidebar" class="sidebar">
    ...
    <div id="layoutRight" class="ncsc-layout-right">
        ...
        <div class="main-content" id="mainContent">
            <div class="tabmenu">
                <ul class="tab pngFix">
                    <li class="<?php if($status == -1): ?>active<?php else: ?>normal<?php endif; ?>"><a  href="<?php echo U('Invoice/index',['status'=>-1]); ?>">所有发票</a></li>
                    <li class="<?php if($status == 0): ?>active<?php else: ?>normal<?php endif; ?>"><a  href="<?php echo U('Invoice/index',['status'=>'0']); ?>">待开</a></li>
                    <li class="<?php if($status == 1): ?>active<?php else: ?>normal<?php endif; ?>"><a  href="<?php echo U('Invoice/index',['status'=>1]); ?>">已开</a></li>
                </ul>
           </div>
            <form method="get" action="<?php echo U('seller/Invoice/index'); ?>" target="_self">
                <input  class="text w70" type="hidden" name="status" value="<?php echo $status; ?>" />
                <table class="search-form">
                    <tr> 
                        <td>&nbsp;</td>
                        <th>创建时间</th>
                        <td class="w378"><input type="text" class="text w70" name="start_time" id="start_time" value="<?php echo $start_time; ?>" />
            ...
            <table class="ncsc-default-table order">
                ...
                <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$vo): ?>
                    <tbody>
                        ...
                        <tr>
                            <th colspan="20"><span class="ml10">订单编号：<em><?php echo $vo['order']['order_sn']; ?></em> </span> <span>创建时间：<em class="goods-time"><?php echo date('Y-m-d H:i:s',$vo['ctime']); ?></em></span> </th>
                        </tr>
                    <?php $goodsList = $goodsArr[$vo['order_id']]; ?>
                    <tr>
                        <td class="bdl"> 
	                        <?php if($list['invoice_type'] == 1): ?> 电子发票
	                        <?php elseif($list['invoice_type'] == 2): ?>增值税发票
	                        <?php else: ?>普通发票 <?php endif; ?>
                        </td>
                    <td class="bdl"><dt><?php echo $vo['invoice_money']; ?></dt></td>
                    <td class="bdl"><dt><?php echo $vo['invoice_title']; ?></dt></td>
                    <td class="bdl"><dt><?php echo $vo['invoice_desc']; ?></dt></td>
                    <td><p><?php echo $vo['invoice_rate']; ?></p></td>
                    <td><?php echo $vo['taxpayer']; ?></td>
                    <td class="bdl"><?php echo $vo['user']['nickname']; ?></td>
                    <td class="bdl"><p class="ncsc-order-amount"><?php if($vo['status'] == 1): if(!(empty($vo['atime']) || (($vo['atime'] instanceof \think\Collection || $vo['atime'] instanceof \think\Paginator ) && $vo['atime']->isEmpty()))): ?><?php echo date('Y-m-d H:i:s',$vo['atime']); endif; endif; ?></p>
                        <p class="goods-pay" title="支付方式："></p></td>
                  
                    <td class="bdl bdr">
	                    <?php if($vo['status'] == 1): ?>
	                        <img width="20" height="20" src="/public/images/yes.png" onclick="changeTableVal2('invoice','invoice_id',<?php echo $vo[invoice_id]; ?>,'status',this);changetime(<?php echo $vo[invoice_id]; ?>);"/>
	                    <?php else: ?>
	                        <img src="/public/images/cancel.png" onclick="changeTableVal2('invoice','invoice_id',<?php echo $vo[invoice_id]; ?>,'status',this);changetime(<?php echo $vo[invoice_id]; ?>);" width="20" height="20">
	                    <?php endif; ?>
                    </td>
                    </tr>
                    </tbody>
...
<script>
    ...
    function changetime(invoice_id){
        $.post("<?php echo U('Invoice/changetime'); ?>",{invoice_id:invoice_id},function(){});
    }
